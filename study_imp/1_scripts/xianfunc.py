...
################################################################################################################
################################################################################################################
############### This library of helper functions are written by Xian Li at JHU 09/2020-04/2023 #################
################################################################################################################
...
def plot_label(x_coords,y_coords,labels='default',font_size=15,xloc=-1,yloc=0.1,add=False,title=False,outpath=False):
    import matplotlib.pyplot as plt
    if labels=='default':
    ...
    for i in range(len(x_coords)):
        ...
        plt.text(x+xloc,y+yloc,lab,size=font_size)
    #show and save figure
    if outpath!=False:
        if title!=False:
            plt.savefig(outpath+title+'.png')  
...
def plot_line(y,x='Default',y_lab='None',x_lab='None',legend='Default',title='None',start=1,add=False,outpath=False,ylim=False):
...
def plot_2dmat(mat,xylab=False,title='ISC_Time-Time-Corr',fontsize=20,figsize=(12,10),outpath=False,save_matrix=False,add=False):
...
def plot_matrix(mat,title=False,outpath=False,xylab=False,labname=False,diagmask=False,map_color="YlGnBu",minmax=(None,None),fontsize=20,save_matrix=False):
...
def plot_swarm(data_as_dict,y,x,outpath=False,hue=False,size=7,alpha=1,edgecolor='black',dodge=False,add=False):
...
def plot_violin(data_as_dict,y,x,outpath=False,color=False,split=False,cut=3,linewidth=None,inner='box',swarm=False,bandwidth=False,palet='muted',alpha=1,add=False):
...
def cyoa_get_merge(mapfile,quiry_events,col_num=[1,2,10],merge='Y',pop_choice=False,choice_marker='_'):
    ...
    #quiry_events: the list of event_lab that you want to know the converge (Y/N) info for.
...
def isc_cent(allsub_cents,method='pairwise',mute=True,skip_nan_min=30):
    ...
    rval,pval = [],[]
    if method=='pairwise':
        rmat,pmat=get_paircorr(allsub_cents,allsub_cents)
...
def sample_one_val_per_key(data_dict,nsample=1):
    ...
    import random    
    out_dict = xf.init_dict(['s'+str(i+1) for i in range(nsample)])
...
def nifti_math(nii_f1,nii_f2=False,math='sub',outpath='default'):
    ...
    #if math=='sub': returns the difference  map of: nii_f1 - nii_f2
    import os
    if nii_f2!=False:
        if outpath=='default':
...
def view_brain(brain_img,surf_type='smooth',view_list=['lateral', 'medial', 'dorsal', 'ventral', 'anterior', 'posterior'],hemi_list=['left','right'],title=False,outpath=False,thresh=1.,view=True,fig_format='.png',vmax=None):
    ...
    for hemi in hemi_list:
        ...
        for view in view_list:
            ...
            fig = plotting.plot_surf_stat_map(surf, texture, hemi=hemi, view = view, title=fname, output_file=out_target,threshold=thresh, bg_map=surf_bg,vmax=vmax)
            if view==True:
                fig.show()
    return 'Completed.'
